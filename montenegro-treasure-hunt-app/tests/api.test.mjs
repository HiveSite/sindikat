...
const port=31991,base=`http://127.0.0.1:${port}`,basePath='/hunt',dbFile=path.resolve('data/test-api.sqlite');
let child;
async function wait(){for(let i=0;i<60;i++){try{const r=await fetch(base+basePath+'/api/health');if(r.ok)return}catch{}await new Promise(r=>setTimeout(r,100))}throw new Error('server nije startovao')}
async function request(p,o={}){const r=await fetch(base+basePath+p,{...o,headers:{'content-type':'application/json',...(o.headers||{})}});const d=await r.json();if(!r.ok)throw new Error(`${p}: ${JSON.stringify(d)}`);return {d,r}}
test.before(async()=>{for(const s of ['', '-shm','-wal'])fs.rmSync(dbFile+s,{force:true});child=spawn(process.execPath,['server.mjs'],{cwd:path.resolve('.'),env:{...process.env,NODE_ENV:'development',PORT:String(port),APP_ORIGIN:base,BASE_PATH:basePath,DATABASE_FILE:dbFile,ENABLE_DEV_TEST_VOUCHER:'true'},stdio:'ignore'});await wait()});
