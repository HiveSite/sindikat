(() => {
  ...
  window.fetch=(input,init={})=>{
    ...
    return typeof input==='string'?nativeFetch(next,init):nativeFetch(new Request(next,input),init);
  };
  ...
  async function loadPrograms(){
    ...
    programs=d.programs||[];
    defaultProgramId=d.defaultProgramId||programs[0]?.id||'PG26';
  ...
  function injectSwitcher(){
    if(!programs.length)return;const side=$('#sidebar');if(!side)return;
  ...
  function patchCopy(){
    ...
    const eye=$('.side-brand .eyebrow');if(eye)eye.textContent=`MTH / ${p.id}`;
    const he=$('.header-copy .eyebrow');if(he)he.textContent=`${p.name}${p.location?' · '+p.location:''}`;
    const online=$('#headerOnline');if(online){const n=(online.textContent.match(/^\d+/)||['0'])[0];online.textContent=`${n}/${p.teamCount} online`}
    const mt=$('.map-toolbar h3');if(mt)mt.textContent=`${p.name} · live operations`;
    ...
    const reset=$('[data-reset-event]');if(reset)reset.textContent=`Resetuj ${p.id} program`;
  }
  ...
  function openManager(){
    ...
    document.querySelectorAll('[data-toggle]').forEach(b=>b.onclick=async()=>{const p=programs.find(x=>x.id===b.dataset.toggle);if(!p)return;try{await api(`/admin/programs/${encodeURIComponent(p.id)}`,{method:'PUT',body:{enabled:!p.enabled}});await loadPrograms();openManager()}catch(e){alert(e.message)}});
  }

  async function createProgram(e){
    e.preventDefault();const err=$('#pError');err.textContent='';
    try{
      const body={id:$('#pId').value,name:$('#pName').value,location:$('#pLocation').value,teamCount:Number($('#pTeams').value),codePrefix:$('#pPrefix').value,cloneFrom:$('#pClone').value};
      const d=await api('/admin/programs',{method:'POST',body});await activate(d.program.id);
    }catch(x){err.textContent=x.message}
  }

  function openCodes(id){
    const p=programs.find(x=>x.id===id);if(!p)return;const dialog=$('.mth-prog-dialog');
    dialog.innerHTML=`<div class="mth-prog-head"><div><small>${esc(p.id)}</small><h2>Pristupni kodovi</h2></div><button class="mth-prog-close" data-back>×</button></div><div class="mth-prog-body"><div class="mth-code-editor"><p>Jedan kod po redu - red 1 je Team 01, red 2 Team 02 itd.</p><label>Kodovi<textarea id="codeList">${p.codes.map(x=>esc(x.code)).join('\n')}</textarea></label><label>Prefix za automatsku regeneraciju<input id="codePrefix" value="${esc(p.codePrefix)}"></label><div class="mth-code-actions"><button id="saveCodes">Sačuvaj kodove</button><button class="secondary" id="regenCodes">Generiši ponovo</button><button class="secondary" id="backPrograms">Nazad</button></div><div class="mth-prog-error" id="codeError"></div></div></div>`;
    const back=()=>openManager();$('[data-back]').onclick=back;$('#backPrograms').onclick=back;
    $('#saveCodes').onclick=async()=>{const err=$('#codeError');err.textContent='';try{const codes=$('#codeList').value.split(/\r?\n/).map(x=>x.trim()).filter(Boolean);await api(`/admin/programs/${encodeURIComponent(id)}/codes`,{method:'PUT',body:{codes}});await loadPrograms();openManager()}catch(e){err.textContent=e.message}};
    $('#regenCodes').onclick=async()=>{const err=$('#codeError');err.textContent='';try{await api(`/admin/programs/${encodeURIComponent(id)}/regenerate-codes`,{method:'POST',body:{codePrefix:$('#codePrefix').value}});await loadPrograms();openCodes(id)}catch(e){err.textContent=e.message}};
  }

  new MutationObserver(schedule).observe(document.documentElement,{childList:true,subtree:true});
  document.addEventListener('DOMContentLoaded',schedule);
  loadPrograms().catch(e=>console.error('MTH programs',e));
})();
