...
async function mthRequest(path,options={}){
  ...
  try{
    ...
    if(error instanceof TypeError)throw new Error('Nije moguće povezati se sa serverom. Provjerite internet i osvježite stranicu.');
    throw error;
...
function mthStatusMarkup(){
  ...
  const message=ready?'Sistem je spreman za aktivaciju.':mthHealthError||'Server još nije odgovorio. Osvježite stranicu ili pokušajte ponovo.';
...
function mthEnhanceLanding(){
  ...
  if(!form)return;
  form.dataset.mthHotfix='1';
  const firstField=form.querySelector('.field');
  let status=form.querySelector('.mth-system-status');
  if(!status&&firstField){firstField.insertAdjacentHTML('beforebegin',mthStatusMarkup());status=form.querySelector('.mth-system-status')}
  if(status){status.outerHTML=mthStatusMarkup()}
  if(mthHealth?.devMode&&!form.querySelector('.mth-test-box')){
