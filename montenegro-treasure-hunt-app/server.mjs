...
async function api(req,res,url) {
  ...
  try {
    ...
    if (p==='/api/admin/login' && req.method==='POST') {
      ...
      return json(res,200,{admin:a},{'Set-Cookie':cookie('mth_admin',token,{sameSite:'Strict',secure:config.isProd,maxAge:config.adminSessionHours*3600})});
    }
    if (p==='/api/admin/logout' && req.method==='POST') {
      const raw=parseCookies(req.headers.cookie).mth_admin; deleteAdminSession(raw);
      return json(res,200,{ok:true},{'Set-Cookie':cookie('mth_admin','',{sameSite:'Strict',secure:config.isProd,maxAge:0})});
    }
    ...
    if (preview && req.method==='POST') { const a=requireAdmin(req,res); if(!a)return; const x=createPreviewAccess(preview[1],a.id); return json(res,201,{...x,url:`/?access=${encodeURIComponent(x.token)}`}); }
...
const server=http.createServer((req,res)=>{const url=new URL(req.url,config.appOrigin);if(url.pathname.startsWith('/api/'))return api(req,res,url);serveStatic(req,res,url);});
