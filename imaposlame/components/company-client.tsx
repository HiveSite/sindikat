...
export function CompanyClient({ view }: { view: "dashboard" | "jobs" | "new-job" | "selection" | "billing" }) {
  ...
  async function saveCompany(formData: FormData) {
    ...
    setMessage(result.error?.message || "Profil firme je sačuvan i čeka odobrenje ako je nov.");
    await load();
  }
  ...
  async function orderPlan(plan: Plan, formData: FormData) {
    ...
    if (orderResult.error || !orderResult.data) return setMessage(orderResult.error?.message || "Narudžba nije kreirana.");
    ...
    setMessage(proofResult.error?.message || `Dokaz je poslat. Poziv na broj: ${orderResult.data.payment_reference}`);
    await load();
  }
    ...
    return (
      <section>
        ...
        <form className="form-card" action={saveCompany}>
          ...
          {company && !company.approved ? <p className="notice">Profil firme čeka odobrenje prije javnog prikaza.</p> : null}
  ...
  if (view === "selection") {
    return (
      <section>
        ...
        <div className="kanban-wrap"><div className="kanban">{stageOrder.map((stage) => <div className="kanban-column" key={stage}><div className="kanban-head"><h4>{stageLabels[stage]}</h4><span className="badge gray">{applications.filter((app) => app.stage === stage).length}</span></div>{applications.filter((app) => app.stage === stage).map((app) => <div className="candidate-card" key={app.id}><strong>{app.profiles?.full_name || app.profiles?.email || "Kandidat"}</strong><p>{app.jobs?.title}</p><p>{app.cover_letter}</p><div className="candidate-cv-summary"><strong>Biografija</strong><p>{app.profiles?.cv_data?.summary || "Kandidat još nije upisao kratak opis."}</p><p>{app.profiles?.phone || ""} {app.profiles?.city || ""}</p></div><div className="candidate-actions">{stageOrder.map((target) => <button className="btn ghost xs" key={target} onClick={() => moveStage(app.id, target)}>{stageLabels[target]}</button>)}</div></div>)}</div>)}</div></div>
        {message ? <p className="notice">{message}</p> : null}
      </section>
    );
  }
  ...
  if (view === "billing") {
    return (
      <section>
        <div className="section-head"><div><span className="page-label">Pretplata</span><h1>Planovi i uplata</h1><p className="sub">Izaberi plan, uplati ručno i pošalji dokaz.</p></div></div>
        <div className="notice"><strong>Podaci za uplatu</strong><br />Primalac: imaposla.me<br />Svrha: Aktivacija plana za firmu {company.name}<br />Poziv na broj dobijaš nakon slanja dokaza.</div>
        <div className="grid three with-top-space">
          {plans.map((plan) => (
            <form className="plan-card" key={plan.id} action={(formData) => orderPlan(plan, formData)}>
              ...
              <button className="btn blue sm">Pošalji dokaz</button>
            </form>
          ))}
        </div>
        <div className="section-head"><div><span className="page-label">Status</span><h1>Moje uplate</h1></div></div>
        <div className="table-card">
          {orders.map((order) => <div className="table-row" key={order.id}><div><strong>{order.payment_reference}</strong><small>{order.plans?.name || "Plan"}</small></div><div>{order.amount_eur} EUR</div><div>{order.status}</div><div>{order.activation_code || "Čeka potvrdu"}</div></div>)}
          {!orders.length ? <div className="empty"><strong>Nema uplata</strong><p>Prva uplata će se prikazati kada pošalješ dokaz.</p></div> : null}
        </div>
