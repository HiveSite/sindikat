...
export async function exportToPNG(element, filename, statusElement, options = {}) {
	...
	try {
		...
		if (options.download === false || options.returnCanvas === true) {
			return finalCanvas;
		}
		...
		alert('Izvoz nije uspio. Provjeri internet konekciju i pokušaj ponovo.');
	} finally {
...
export async function exportToPDF(element, filename, statusElement) {
	const jsPDF = window.jspdf && window.jspdf.jsPDF;
	if (!jsPDF) {
		alert('PDF modul nije učitan. Provjeri internet konekciju i pokušaj ponovo.');
		return;
	}

	const canvas = await exportToPNG(element, filename.replace(/\.pdf$/i, '.png'), statusElement, {
		download: false,
		returnCanvas: true
	});
	if (!canvas) return;
