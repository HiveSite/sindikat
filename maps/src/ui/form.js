...
export function setupControls() {
	...
	const fitCityBtn = document.getElementById('fit-city-btn');
	const centerMarkerBtn = document.getElementById('center-marker-btn');
	...
	const layerParks = document.getElementById('layer-parks');
	const layerTerrain = document.getElementById('layer-terrain');
	const layerRail = document.getElementById('layer-rail');
	const layerBoundaries = document.getElementById('layer-boundaries');
	const roadWeightSlider = document.getElementById('road-weight-slider');
	const roadWeightValue = document.getElementById('road-weight-value');
	...
	const grainOpacitySlider = document.getElementById('grain-opacity-slider');
	const grainOpacityValue = document.getElementById('grain-opacity-value');
	...
	if (artisticMainGrid) {
		const mainKeys = ['studio_paper', 'studio_noir', 'studio_blueprint'];

		const makeCard = (key, theme, isOther = false) => {
			const p = paletteFor(theme);
			const label = theme && theme.name ? theme.name : (isOther ? 'Ostale teme' : key);
			return `
		...
		const mainHtml = mainKeys.map(k => makeCard(k, artisticThemes[k] || {})).join('') + makeCard('other', { name: 'Ostale teme' }, true);
	...
	function openCustomThemeEditor(key = null) {
		...
		ctModalTitle.textContent = key ? 'Uredi prilagođenu temu' : 'Kreiraj prilagođenu temu';
	...
	function populateArtisticModal() {
		...
		const mainKeys = new Set(['studio_paper', 'studio_noir', 'studio_blueprint']);
		...
		const customSection = customKeys.length ? `
			<div class="space-y-2 pb-4 border-b border-slate-100">
				<div class="flex items-center justify-between">
					<p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Moje teme</p>
					<div class="flex items-center gap-2">
						<button id="export-custom-themes-btn" class="text-[10px] font-bold text-slate-400 hover:text-accent transition-colors flex items-center gap-1" title="Izvezi sve prilagođene teme kao JSON">
							<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
							Izvezi
						</button>
						<button id="delete-all-custom-themes-btn" class="text-[10px] font-bold text-slate-400 hover:text-red-500 transition-colors flex items-center gap-1" title="Obriši sve prilagođene teme">
					...
					<div class="flex items-center gap-2 p-3 border border-slate-100 rounded-2xl hover:shadow-md transition-all" data-search-row>
						<button class="artistic-modal-item flex-1 flex items-center gap-3 text-left" data-key="${key}">
							...
							<div>
								...
								<div class="text-[10px] text-slate-400 mt-0.5">${t.description || 'Prilagođena tema'}</div>
							</div>
						</button>
						<button class="edit-custom-btn shrink-0 w-8 h-8 flex items-center justify-center rounded-xl hover:bg-slate-100 transition-colors" data-key="${key}" title="Uredi">
		...
		artisticModalContent.innerHTML = `
			<div class="flex gap-2">
				<button id="create-custom-theme-btn" class="flex-1 flex items-center gap-2 p-3.5 border-2 border-dashed border-slate-200 rounded-2xl hover:border-accent hover:bg-accent/5 transition-all text-slate-400 hover:text-accent">
					...
					<span class="text-xs font-semibold">Kreiraj temu</span>
				</button>
				<button id="import-custom-themes-btn" class="flex items-center gap-2 px-4 py-3.5 border-2 border-dashed border-slate-200 rounded-2xl hover:border-accent hover:bg-accent/5 transition-all text-slate-400 hover:text-accent" title="Uvezi teme iz JSON fajla">
					...
					<span class="text-xs font-semibold">Uvezi</span>
				</button>
			</div>
			<input type="file" id="import-custom-themes-file" accept=".json,application/json" class="hidden" />
			${customSection}
			<div class="mb-2">
				<input id="artistic-search" type="search" placeholder="Pretraži teme..." class="w-full input-field" />
			</div>
		...
		document.getElementById('import-custom-themes-file')?.addEventListener('change', (e) => {
			...
			reader.onload = (ev) => {
				try {
					...
					populateArtisticModal();
					alert(`Uvezeno tema: ${imported}${skipped ? ` - preskočeno neispravnih: ${skipped}` : ''}.`);
				} catch {
					alert('Fajl nije moguće pročitati. Provjeri da li je validan JSON izvoz iz ove aplikacije.');
				}
				e.target.value = '';
			};
		...
		document.getElementById('delete-all-custom-themes-btn')?.addEventListener('click', () => {
			if (!confirm(`Obrisati sve prilagođene teme (${customKeys.length})? Ova radnja se ne može vratiti.`)) return;
			clearCustomThemes();
			if (customKeys.includes(state.artisticTheme)) updateState({ artisticTheme: 'studio_paper' });
			populateArtisticModal();
		});
    ...
    `).join('');
		...
		modalContent.innerHTML = `
			<div class="mb-4">
				<input id="preset-search" type="search" placeholder="Pretraži formate i dimenzije..." class="w-full input-field" />
			</div>
	...
	searchInput.addEventListener('input', (e) => {
		...
		searchTimeout = setTimeout(async () => {
		  ...
		  <div class="search-result-item px-4 py-3 cursor-pointer text-sm rounded-xl" data-lat="${r.lat}" data-lon="${r.lon}" data-name="${r.shortName}" data-country="${r.country || ''}" data-south="${r.bounds?.south ?? ''}" data-north="${r.bounds?.north ?? ''}" data-west="${r.bounds?.west ?? ''}" data-east="${r.bounds?.east ?? ''}">
			<div class="font-semibold">${r.shortName || r.name}</div>
			<div class="text-[10px] opacity-60 mt-0.5">${r.name}</div>
		  </div>
		`).join('');
	...
	function selectResultElement(item) {
		...
		const boundsValues = [item.dataset.south, item.dataset.north, item.dataset.west, item.dataset.east].map(Number);
		const searchBounds = boundsValues.every(Number.isFinite) ? { south: boundsValues[0], north: boundsValues[1], west: boundsValues[2], east: boundsValues[3] } : null;
	...
	if (bearingSlider) {
		bearingSlider.addEventListener('input', (e) => {
			const bearing = Number(e.target.value || 0);
			updateState({ bearing, renderMode: 'artistic' });
	...
	if (pitchSlider) {
		pitchSlider.addEventListener('input', (e) => {
			const pitch = Number(e.target.value || 0);
			updateState({ pitch, renderMode: 'artistic' });
			updateMapCamera({ pitch });
		});
	}

	resetViewBtn?.addEventListener('click', () => {
		updateState({ zoom: 13, bearing: 0, pitch: 0 });
		updateMapCamera({ zoom: 13, bearing: 0, pitch: 0 });
	});

	fitCityBtn?.addEventListener('click', () => fitMapBounds(state.searchBounds));

	centerMarkerBtn?.addEventListener('click', () => {
		const markers = [...(state.markers || [])];
		if (markers.length) markers[0] = { lat: state.lat, lon: state.lon };
		else markers.push({ lat: state.lat, lon: state.lon });
		updateState({ markers, showMarker: true });
		updateMarkerStyles(state);
	});

	const layerBindings = [
		[layerRoads, 'showRoads'],
		[layerBuildings, 'showBuildings'],
		[layerWater, 'showWater'],
		[layerParks, 'showParks'],
		[layerTerrain, 'showTerrain'],
		[layerRail, 'showRail'],
		[layerBoundaries, 'showBoundaries']
	];
	layerBindings.forEach(([control, key]) => {
		control?.addEventListener('change', (e) => {
			updateState({ [key]: e.target.checked, renderMode: 'artistic' });
			updateArtisticLayerSettings(state);
		});
	});

	roadWeightSlider?.addEventListener('input', (e) => {
		updateState({ roadWeight: Number(e.target.value) / 100, renderMode: 'artistic' });
		updateArtisticLayerSettings(state);
	});
	buildingOpacitySlider?.addEventListener('input', (e) => {
		updateState({ buildingOpacity: Number(e.target.value) / 100, renderMode: 'artistic' });
		updateArtisticLayerSettings(state);
	});
	terrainOpacitySlider?.addEventListener('input', (e) => {
		updateState({ terrainOpacity: Number(e.target.value) / 100, renderMode: 'artistic' });
		updateArtisticLayerSettings(state);
	});
	grainOpacitySlider?.addEventListener('input', (e) => {
		updateState({ grainOpacity: Number(e.target.value) / 100 });
	});
	...
	function doResetSettings() {
		if (confirm('Resetovati sva podešavanja?')) {
	...
	return (currentState) => {
		...
		if (bearingSlider) bearingSlider.value = currentState.bearing || 0;
		if (bearingValue) bearingValue.textContent = `${Math.round(currentState.bearing || 0)}°`;
		if (pitchSlider) pitchSlider.value = currentState.pitch || 0;
		if (pitchValue) pitchValue.textContent = `${Math.round(currentState.pitch || 0)}°`;
		const syncChecks = [
			[layerRoads, currentState.showRoads !== false],
			[layerBuildings, currentState.showBuildings !== false],
			[layerWater, currentState.showWater !== false],
			[layerParks, currentState.showParks !== false],
			[layerTerrain, currentState.showTerrain !== false],
			[layerRail, currentState.showRail !== false],
			[layerBoundaries, currentState.showBoundaries !== false]
		];
		syncChecks.forEach(([control, checked]) => { if (control) control.checked = checked; });
		if (roadWeightSlider) roadWeightSlider.value = Math.round((currentState.roadWeight || 1.15) * 100);
		if (roadWeightValue) roadWeightValue.textContent = `${(currentState.roadWeight || 1.15).toFixed(2)}x`;
		if (buildingOpacitySlider) buildingOpacitySlider.value = Math.round((currentState.buildingOpacity ?? 0.9) * 100);
		if (buildingOpacityValue) buildingOpacityValue.textContent = `${Math.round((currentState.buildingOpacity ?? 0.9) * 100)}%`;
		if (terrainOpacitySlider) terrainOpacitySlider.value = Math.round((currentState.terrainOpacity ?? 0.82) * 100);
		if (terrainOpacityValue) terrainOpacityValue.textContent = `${Math.round((currentState.terrainOpacity ?? 0.82) * 100)}%`;
		if (grainOpacitySlider) grainOpacitySlider.value = Math.round((currentState.grainOpacity ?? 0.1) * 100);
		if (grainOpacityValue) grainOpacityValue.textContent = `${Math.round((currentState.grainOpacity ?? 0.1) * 100)}%`;
...
export function updatePreviewStyles(currentState) {
	...
	const posterNoise = document.getElementById('poster-noise');
	...
	if (posterNoise) posterNoise.style.opacity = String(currentState.grainOpacity ?? 0.1);
