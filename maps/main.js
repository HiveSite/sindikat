...
import { initMap, updateMapTheme, invalidateMapSize, waitForTilesLoad, waitForArtisticIdle, updateMarkerStyles, updateRouteStyles, updateArtisticLayerSettings } from './src/map/map-init.js';
import { setupControls, updatePreviewStyles } from './src/ui/form.js';
import { exportToPNG, exportToPDF } from './src/core/export.js';
...
subscribe((currentState) => {
	...
	updateArtisticLayerSettings(currentState);
		...
		if (loading) {
			exportBtn.innerHTML = `
				<div class="flex items-center justify-center space-x-3">
					...
					<span>${mode === 'processing' ? 'Obrada...' : 'Učitavanje...'}</span>
				</div>
			`;
		} else {
...
exportBtn.addEventListener('click', async () => {
	const filename = `Studio83-Mapa-${state.city.replace(/\s+/g, '-')}-${Date.now()}.png`;
...
exportPdfBtn?.addEventListener('click', async () => {
	const filename = `Studio83-Mapa-${state.city.replace(/\s+/g, '-')}-${Date.now()}.pdf`;
	exportPdfBtn.disabled = true;
	exportPdfBtn.classList.add('opacity-60', 'cursor-not-allowed');
	try {
		await exportToPDF(posterContainer, filename, document.getElementById('export-status'));
	} finally {
		exportPdfBtn.disabled = false;
		exportPdfBtn.classList.remove('opacity-60', 'cursor-not-allowed');
	}
});

mobilePdfBtn?.addEventListener('click', async () => {
	const filename = `Studio83-Mapa-${state.city.replace(/\s+/g, '-')}-${Date.now()}.pdf`;
	mobilePdfBtn.disabled = true;
	try {
		await exportToPDF(posterContainer, filename, document.getElementById('export-status'));
	} finally {
		mobilePdfBtn.disabled = false;
	}
});
