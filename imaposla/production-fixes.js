(() => {
  ...
  function installDataOverrides() {
    ...
    loadData = async function loadDataFixed() {
      ...
      if (state.profile?.role === 'candidate') state.applications = await safeQuery(() => db.from('job_applications').select('*,jobs(title,companies(name))').eq('candidate_id', state.user.id).order('created_at', { ascending: false }));
    ...
    jobDetail = function jobDetailFixed(raw) { originalJobDetail(raw); setTimeout(markDuplicateApplications, 20); };
    loadData().then(() => { if (typeof render === 'function') render(); runVisualFixes(); });
  }
  ...
  async function fixHomeLatestJobs() {
    ...
    const next = jobs.length ? jobs.map(job => `<a class="job-card" href="#/oglasi/${slugText(job.title)}-${job.id}"><span class="kicker">${h(job.city || 'Crna Gora')}</span><h3>${h(job.title || 'Oglas za posao')}</h3><p>${h(job.company || 'Firma')} · ${h(job.category || 'Kategorija')} · ${h(job.type || 'Dogovor')}</p><strong>${h(job.salary || 'Plata po dogovoru')}</strong></a>`).join('') : `<div class="empty home-empty"><h3>Još nema aktivnih oglasa</h3><p>Kada firma pošalje oglas i bude odobren, pojaviće se ovdje.</p><div class="actions"><a class="btn blue" href="#/oglasi">Pretraga oglasa</a><a class="btn lime" href="#/login?mode=signup&role=company">Objavi oglas</a></div></div>`;
  ...
  function addPasswordResetLink() {
    ...
    button.type = 'button'; button.className = 'btn ghost sm'; button.dataset.resetPassword = 'true'; button.textContent = 'Zaboravljena lozinka';
  ...
  document.addEventListener('click', async (event) => {
    ...
    if (reject && hasCore()) {
      ...
      await loadData(); showToast('Prijava je označena kao odbijena.'); render(); return;
    }
  ...
  function runVisualFixes() { fixHomeLatestJobs(); addPasswordResetLink(); markDuplicateApplications(); }
  function run() { installDataOverrides(); runVisualFixes(); }
  function scheduleVisualFixes() { clearTimeout(window.productionVisualTimer); window.productionVisualTimer = setTimeout(runVisualFixes, 120); }

  window.addEventListener('DOMContentLoaded', () => {
    [50, 300, 700, 1500, 2600].forEach(ms => setTimeout(run, ms));
    new MutationObserver(scheduleVisualFixes).observe(document.body, { childList: true, subtree: true });
  });
  window.addEventListener('hashchange', () => [120, 500, 1200].forEach(ms => setTimeout(run, ms)));
})();
